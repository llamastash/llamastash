#!/usr/bin/env bash
# End-to-end UAT for the SGLang backend against a real SGLang.
#
...
#
# Usage: scripts/sglang/uat.sh <clean|boot|launch|replay|preset|chat|stop|all>
set -euo pipefail

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
cd "$ROOT"

UAT_HOME="${LS_UAT_HOME:-$HOME/.cache/ls-sglang-e2e}"
...
PROXY_PORT="${LS_UAT_PROXY_PORT:-21436}"
...
# Every accessor selects by LaunchId, so a later stage can never report an
# earlier launch's argv as its own result.
...
do_boot() {
  ...
  local avail
  avail=$($LS status --json | jq -r '.backends[] | select(.id=="sglang") | .enabled')
  [ "$avail" = "true" ] || die "the sglang backend is not available — set backend.sglang.servers[0].binary"
...
do_launch() {
  local id; id=$(start_and_capture $LS start "$MODEL" --backend sglang --ctx 3072 --mode chat)
  sleep 4
  local cap; cap=$(flag_val "$id" --max-total-tokens)
  [ -n "$cap" ] || die "no token cap in argv — SGLang would size its pool from the memory fraction, which on a unified host is system RAM"
  ok "token cap $cap"
  wait_ready "$id"
  ...
  [ "$ctx" = "3072" ] || die "resolved_ctx=$ctx, expected 3072 (read from /get_server_info)"
  ok "resolved_ctx $ctx"
...
do_replay() {
  stop_all
  local id; id=$(start_and_capture $LS start "$MODEL" --backend sglang)
  sleep 4
  local cl; cl=$(flag_val "$id" --context-length)
  [ "$cl" = "3072" ] || die "last_params did not replay --ctx (got '${cl:-none}')"
  ok "last_params replayed ctx $cl"
  [ -n "$(flag_val "$id" --max-total-tokens)" ] || die "token cap missing on the replayed launch"
  ok "token cap re-resolved"
  wait_ready "$id"
}

do_preset() {
  stop_all
  local id; id=$(start_and_capture $LS start "$MODEL" --backend sglang --preset sglang-small)
  sleep 4
  local cap; cap=$(flag_val "$id" --max-total-tokens)
  [ "$cap" = "8192" ] || die "preset cap not honoured (got '${cap:-none}', expected 8192)"
...
do_chat() {
  local id port code
  id=$(cat "$UAT_HOME/.last_id" 2>/dev/null) || die "no launch recorded — run the launch stage first"
