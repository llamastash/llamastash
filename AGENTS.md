...
1. `src/backend/<id>/` — **all** its logic, behind trait methods: argv/launch translation, `resolve_launch_binary`, identity, capabilities, native knobs, `seed_launch_knobs`, `auto_routes`, `serves_mode`, `serves_web_ui`, `refuses`, `kv_bytes`, `process_markers` / `argv_is_server`, availability (`available` / `installed` / `status_*`), and — for a managed multiplexer — the `start` / `stop` overrides, `umbrella_launch_id`, and `supervise_at_boot`. Delegation specifics (umbrella-unload, "what's resident") stay private to the module. A process-per-model backend leaves `start` / `stop` on their defaults and never touches lifecycle plumbing.
...
- **Four backends; llama.cpp is the stable default.** Lemonade, ds4 and vLLM are experimental and default-on only when their binary resolves. A ds4-compatible GGUF that can't use ds4 **falls back to llama.cpp — never a refusal**. R13 ("a disk GGUF binds llama.cpp") has exactly this one exception — vLLM is not another: it claims safetensors repos, never GGUF. → `docs/architecture.md` § Backends.
