...
/// Whether a safetensors-serving engine can take `candidate`.
///
/// Deliberately permissive: safetensors weights present and no GGUF in the
/// repo (a mixed repo belongs to the GGUF scanner). Engines refuse some
...
/// fails at launch with the engine's own diagnostic rather than being hidden.
pub fn serves_safetensors_only(candidate: &HfRepoCandidate) -> bool {
  candidate.has_safetensors && !candidate.has_gguf
}

/// Project an eligible candidate into a catalog row claimed by `backend_id`.
///
/// The row's `path` is the **snapshot directory**, not a file: the engine is
/// handed the directory and resolves weights itself. `ModelSource` stays
...
pub fn project_safetensors_row(candidate: &HfRepoCandidate, backend_id: &str) -> DiscoveredModel {
  let weights = weights_bytes(&candidate.snapshot_path);
  let metadata = candidate
    .config_summary
    .as_ref()
    .map(|s| config_to_metadata(s, &candidate.repo_id))
    ...
    .or_else(|| weights.is_some().then(metadata_without_config))
    .map(|mut m| {
  ...
  DiscoveredModel {
    ...
    // revision hash, so without this every row would read as a sha.
...
/// the on-disk shape a safetensors engine serves, checked without the
/// cache-layout assumptions [`serves_safetensors_only`] gets for free from
/// the enumerator.
...
pub fn repo_id_for_snapshot(snapshot: &Path) -> Option<String> {
  ...
  crate::util::model_caches::repo_id_from_cache_dir(dir_name)
}
...
mod tests {
  ...
  #[test]
  fn serves_safetensors_only_needs_weights_and_no_gguf() {
    assert!(serves_safetensors_only(&candidate(true, false)));
    assert!(
      !serves_safetensors_only(&candidate(false, false)),
      "no weights"
    );
    assert!(
      !serves_safetensors_only(&candidate(true, true)),
  ...
  fn projection_uses_the_snapshot_dir_and_repo_id() {
    let row = project_safetensors_row(&candidate(true, false), "engine");
    assert_eq!(
      row.path,
    ...
    assert_eq!(row.supported_backends, vec!["engine".to_string()]);
  }
  ...
  fn a_config_parse_failure_still_carries_the_size() {
    let dir = crate::util::test_temp::unique_temp_dir("hf-noconfig-size");
    ...
    let row = project_safetensors_row(&c, "engine");
    assert!(
  ...
  fn is_safetensors_snapshot_rejects_a_nested_gguf() {
    let dir = crate::util::test_temp::unique_temp_dir("hf-nested-gguf");
  ...
  fn weights_bytes_sums_shards_and_ignores_other_files() {
    let dir = crate::util::test_temp::unique_temp_dir("hf-weights");
  ...
  fn weights_bytes_is_none_when_there_are_no_safetensors() {
    let dir = crate::util::test_temp::unique_temp_dir("hf-noweights");
    std::fs::write(dir.join("config.json"), b"{}").unwrap();
