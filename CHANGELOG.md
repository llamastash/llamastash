...
- **Run preset files**. `llamastash run` — alias for `start`, and a launch file: `run model.yml` starts the file's one model with its own preset, without saving anything to `config.yaml`. A hand-authored file is checked strictly: an undeclared knob, an unparseable knob value or a misspelled field exits 64 rather than launching without it.
