...
- **Padded + colored CLI tables on TTY for `list`, `status`, `presets list`, `favorites list`, `last-params`, and `daemon status`.** Report-style commands now render column-padded tables with bold headers, a dim underline rule, and semantic value colors (state-green/yellow/dim/red, launch-id magenta, port cyan, paths with `$HOME → ~` collapse). Action-style commands (`daemon start/stop`, `start`, `stop`) pick up value-color highlights on launch-id / port / pid / state. `daemon status` no longer dumps raw pretty-JSON — it now renders the daemon's `version` response as a labelled key/value block. Pipes and `--no-colors` / `NO_COLOR` keep emitting today's exact TSV bytes so `awk -F\t` / `column -t` pipelines stay supported. `--json` is byte-stable everywhere. No new crates — `console` + `cliclack` (both already in tree) cover the new helpers in `src/cli/format.rs`.
