...
- **A daemon crash left a named launch unaddressable.** The surviving `llama-server` was re-adopted as a read-only `external` row with its name dropped, so `stop coder` missed a process that was up and serving. The name now rides onto that row: `status` still prints `<model>@<name>` and `stop` still takes it. The row stays read-only and is not re-published on the proxy — routing needs a supervisor — so a request for that address starts a fresh launch beside it.
