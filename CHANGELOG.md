...
- **llama.cpp's new unified `llama` binary is now found and launched.** Upstream's own installer (`llama.app`, `winget`) ships one `llama` / `llama.exe` that serves behind a `serve` subcommand instead of a standalone `llama-server`, so a fresh install left LlamaStash with nothing to launch. `$PATH` lookup falls back to it, `--llama-server` / `LLAMASTASH_LLAMA_SERVER` / `backend.llamacpp.servers` take it, and the subcommand is added automatically. Closes [#67](https://github.com/llamastash/llamastash/issues/67).
