...
- New `gpu.disable_vulkan_probe` config toggle (default `false`). When `true` the daemon skips the Vulkan fallback probe (`vulkaninfo -j` / `--summary`) — no subprocess spawns at startup or during the periodic 60-s hotplug re-probe. Vulkan-only hosts unaffected; the toggle is for users whose native NVIDIA/AMD probe already covers all GPUs. Config-only, no CLI/env surface.
