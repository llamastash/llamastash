...
- **A launch was refused for memory it never uses.** llama.cpp streams a per-layer embedding table from the mapping instead of holding it resident, but the admission gate priced the whole file: Qwen3.8-Flash-Next (103.7 GiB on disk, 26.8 GiB of it that table) projected 133.5 GiB and was refused on a 121 GiB host that runs it in about 79 GiB. The gate now subtracts what the engine streams, and still counts it in full when the launch pins `--no-mmap` or `--lazy-mode off`, which disable streaming.
