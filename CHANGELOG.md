...
- **A launch was refused for memory it never uses.** llama.cpp streams a per-layer embedding table from the mapping instead of holding it resident, but the admission gate priced the whole file: Qwen3.8-Flash-Next (103.7 GiB on disk, 26.8 GiB of it that table) projected 133.5 GiB and was refused on a 121 GiB host that runs it in about 79 GiB. The gate now subtracts what the engine streams, and still counts it in full when streaming is disabled, whether by the `no-mmap` knob or by `--no-mmap` / `--load-mode` / `--direct-io` / `--lazy-mode off` passed through to the engine.
...
- **A named preset leaked a stale `last_params` into the launch.** `start --preset <name>` and a TUI form launch flattened the preset into the `User` layer but still let the `LastUsed` layer fill in knobs the preset left unset, so a value from an earlier manual launch could override the preset's intent. An explicit selection now resolves as pure-fit and skips the `LastUsed` layer, matching `--preset auto`. The inline-flag-only `start` path (no preset) is unaffected and keeps its `PresetDefault → LastUsed` fallback.
- **A preset saved from the TUI (`Ctrl+P`) dropped the engine and build it ran on.** The save payload carried only knobs and extras, so a TUI-saved preset read back with `server: null` and silently fell to `servers[0]` instead of the build the model was actually running on — for a fork-quantized GGUF that meant the stock binary rather than the fork build it was captured against. The CLI's `presets save` already sent the identity; the TUI path now does too, and a `--device` launch stamps the device-derived server so status (and the capture) report the real build.
...
- **Presets, CLI flags and the Settings editor could disagree about a backend's knobs.** Every tunable is now declared once by the backend that owns it, and the CLI flag, editor row and preset key are generated from that one declaration — so a knob reaches all three surfaces or none. Adds `llamastash knobs` to list them.
- **A preset could silently lose its context window.** On a backend whose context flag is not the one a backend-blind lookup matches first, `ctx:` was stored under a sibling's key and then not found, so the preset launched at the engine default.
