...
| 0 | Legal, documented: rejects every non-empty body (a `Limited` cap of 0 trips on the first byte). No new error type, no validation pass |
...
| 413 message | `request body exceeds the {cap} limit` with a small private `format_bytes` — largest unit the value fits in, whole when exact (`16 MiB`, `1 GiB`), two decimals otherwise with trailing zeros trimmed (`976.57 KiB`, never `2.00 MiB`); sub-KiB stays raw bytes (`512 B`) |
