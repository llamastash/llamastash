...
- [ ] **P0: Wire text-input capture for right-pane tabs.** When `Focus::RightPane` is active, `handle_key` falls through to `action_for(Focus::RightPane, ...)` which silently drops unbound keystrokes. Chat/embed/rerank input fields can never be populated. Fix: add per-tab `Focus` variants (`Focus::ChatInput`, `Focus::EmbedInput`, `Focus::RerankInput`) or a tab-aware text-capture dispatch so typed characters reach `app.chat.prompt`, `app.embed.input`, and `app.rerank.query`. At minimum, forward alphanumeric + backspace + enter to the active tab's input buffer. Cover with a smoke test that types "hello" into the chat prompt and asserts it appears in the render.
- [ ] **P0: Wire `Ctrl+Enter` to send the chat prompt via `spawn_chat_stream`.** The plan specifies `Ctrl+Enter` as the send hotkey but there is no `Action::SendChat` or `Ctrl+Enter` binding in `RIGHT_PANE_BINDINGS`. The `spawn_chat_stream` function exists and the integration test proves it works, but nothing in the TUI run loop calls it. Fix: add a `Ctrl+Enter` binding that reads `app.chat.prompt`, calls `spawn_chat_stream(app.focused_managed().port, focused_name(), prompt)`, drains the returned `mpsc::Receiver` into `app.chat` via a periodic `try_recv` in the render loop, and pushes final state on `Finished`/`Error`. Cover with a TestBackend smoke test that presses `Ctrl+Enter` and asserts the streaming state appears.
- [ ] **P0: Wire `Enter` submit for embed and rerank tabs.** The embed tab status bar says "Enter to embed" and the rerank tab says "Enter to rank", but `Enter` in `Focus::RightPane` dispatches to `action_for` which has no `Submit` binding for the right pane. Fix: add `Enter` bindings for `ChatInput`, `EmbedInput`, `RerankInput` sub-focuses that call `oai_client::embed` / `oai_client::rerank` asynchronously and push results into `app.embed` / `app.rerank`. Cover with smoke tests that assert dim/preview for embed and ranked scores for rerank. (The OAI client primitives already pass integration tests; this is purely wiring them to keyboard input.)
...
- [ ] **P1: Add the `r` hotkey for toggling think-block collapse in the Chat tab.** The plan calls for `r` to toggle per-message ` thinking... response` collapse (R32). `ChatTabState.collapse_thinks` exists and `ooi_client::collapse_think_blocks` works, but there is no `Action::ToggleThinkCollapse` or `r` binding. Add a binding in `RIGHT_PANE_BINDINGS` (scoped to `Focus::ChatInput` or a parallel `ChatView` focus) and wire it to flip `app.chat.collapse_thinks`.
- [ ] **P1: Drain the streamed chat response into `app.chat` via a non-blocking channel poll.** `spawn_chat_stream` returns an `mpsc::Receiver<ChatStreamMsg>` but no task reads from it. The render loop needs a `try_recv` drain path that appends `Delta` to `app.chat.response`, calls `mark_finished` on `Finished`, and shows an error toast on `Error`. Store the receiver handle in `app.chat` (e.g. `stream_rx: Option<mpsc::Receiver<ChatStreamMsg>>`) so it outlives the spawn.
- [ ] **P2: Add multi-field input support to the Rerank tab.** The tab UI shows a query field and candidate list, but `RerankTabState` only has `query: String` and `candidates: Vec<String>` with no per-field cursor state or `Tab`-cycling between them. The status bar says "type candidate text then Tab to stage" but no Tab handler exists. Add a `field` cursor (Query / CandidateInput) to `RerankTabState`, a `Tab` cycle action that stages the current candidate text and clears the input, and render both the query prompt and the in-progress candidate with the caret.
- [ ] **P2: Add an auto-scroll toggle hotkey for the Logs tab.** `LogsTabState.auto_scroll` exists but there is no keybinding to toggle it. Add `Action::ToggleAutoScroll` bound to a key (e.g. `Space` or `s`) and wire it to flip `auto_scroll` when `Focus::RightPane` and `app.right_tab == RightTab::Logs`.
- [ ] **P2: Add per-tab focus states for right-pane text input.** Currently the only right-pane focus is `Focus::RightPane`, but each tab has its own input surface (chat prompt, embed text, rerank query). Adding `ChatInput`, `EmbedInput`, `RerankInput` variants to `Focus` (with a fallback `RightPane` for non-input tab views like logs) lets `handle_key` direct alphanumerics to the right buffer. This is a prerequisite for the P0 text-input fix; file as P2 because the scoping is more complex and can be deferred until the P0 input path is designed.
- [ ] **P2: Add `Ctrl+Enter` and `/` keybindings to the right-pane help bar hints.** The plan documents `Ctrl+Enter` for send and the chat tab status line already mentions it. Add explicit bindings to the `RIGHT_PANE_BINDINGS` table (or the new per-tab focus tables) so the help bar renders them. Also add a binding for `Esc` to clear embed state when the tab already shows results.
