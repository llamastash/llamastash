...
**TODO follow-ups from Unit 6 review (2026-05-15 — TUI shell commits `1db7a2e` + `ddc7f2d`):**

- [x] **P0: Wire an IPC client into the TUI action-dispatch path and send `start_model` on LaunchPicker Submit.** `events::apply_action` currently shows a toast "launch dispatched" for `Focus::LaunchPicker` on `Action::Submit` but never calls the daemon's `start_model` method. The `run` loop in `events.rs` creates a read-only refresher (polling `list_models`/`status`/`favorite_list`) but provides no `mpsc::Sender` or `Client` handle for action dispatch. Fix: thread an `mpsc::Sender<(String, serde_json::Value)>` through `run` into `apply_action`, spawn a writer task that owns a persistent `Client` and forwards requests, and replace the placeholder toast with an async `start_model` call using the assembled `LaunchPickerState` fields. Cover with a smoke test that asserts `start_model` params reach the backend.
- [x] **P0: Wire `favorite_add`/`favorite_remove` IPC calls into `ToggleFavorite`.** The toggle currently mutates `app.favorites` in local memory only; the next 750 ms `favorite_list` refresh from the daemon overwrites the local change, making favorites un-persistable through the TUI. Fix: send `favorite_add` or `favorite_remove` through the same writer channel as the launch fix above, keep the optimistic local toggle, and revert on IPC error. Cover with a smoke test that asserts a favorite round-trip survives refresh.
- [x] **P1: Hit the plan's <16 ms input-to-redraw latency target.** `POLL_INTERVAL` is 40 ms (~25 fps); worst-case key-to-redraw is thus 40 ms. Keep `POLL_INTERVAL` at 8 ms and read the event in non-blocking mode inside a tighter loop, or switch to `event::poll(Duration::ZERO)` after each tick so the render+input cycle is truly bounded by the terminal draw cost. Add a profiling assertion or microbenchmark.
- [x] **P1: Assemble the `LaunchPickerState` and `AdvancedPanelState` fields into a `start_model` IPC payload.** The picker collects `ctx: Option<u32>`, `reasoning: bool`, and the advanced panel's free-form `argv`, but none of these are consumed at Submit time. The IPC payload must include: `path` (from `app.focused_path()`), `ctx` (default to `None` for native), `reasoning` (bool), and `advanced` (string array from `advanced_panel.argv()`). Update `launch_picker::render` to show the currently selected ctx value rather than just preset cycling.
- [x] **P2: Add the plan-mandated est-mem badge to rendered list-pane rows.** The plan calls for rows to show "est-mem badge (KV-aware once ctx is chosen at launch)" alongside arch, quant, and native_ctx. The daemon's `discovery_task` should carry a `MemoryEstimate` in each `DiscoveredModel` (or the catalog JSON), and `list_pane::render_row` should render it as an additional badge (e.g. `· 4.2G` for weights-only, `· 5.8G+KV` when a ctx is chosen). If the daemon IPC surface doesn't yet expose the estimate, file it as a daemon-side item and note the dependency.
- [x] **P2: Resolve the plan's open question about `nucleo-matcher` vs `fuzzy-matcher`.** Unit 6 chose a hand-rolled subsequence ranker (`src/tui/filter.rs`) instead of `nucleo-matcher`. If the hand-rolled ranker is the final choice, close the plan's deferred question with a note in `src/tui/filter.rs` documenting the decision (e.g. "nucleo-matcher rejected to keep dep tree tight; hand-rolled subsequence + contiguous-run bonus is sufficient for model-name filtering"). If performance on large catalogs (>500 models) regresses, reconsider.
- [x] **P2: Pre-populate the launch picker from the daemon's `last_params` store.** The plan says the picker is "pre-populated with last-params and named-preset values." Currently `LaunchPickerState::for_model` always sets `ctx: None` and `reasoning: false`. Read `state.json` `last_params` for the focused model's path (via a new IPC query or an extra field in `list_models`) and seed `ctx` and `reasoning` from the persisted values when they exist. Same for presets — the picker should enumerate saved presets for the focused model and let the user select one.
- [x] **P2: Detect duplicate launches in the picker.** Plan: "launching a model already running → picker pre-populates with that model's last params; submit creates a *new* instance on a new port (no duplicate-prevention in v1)." The picker should surface a heads-up when the focused model has active `ManagedRow`s: show the existing port, state, and the fact that a second instance will be created. A warning line in the picker body is sufficient for v1.
- [x] **P2: Add exponential backoff to the daemon reconnect loop in `spawn_refresher`.** The refresher connects on a fixed 750 ms interval with no backoff; when the daemon is down the rate is `1/0.75 ≈ 1.3 Hz`. Use `tokio::time::sleep(backoff)` where `backoff` starts at 750 ms and doubles on each failure up to a cap of `REFRESH_INTERVAL` (so a freshly-started daemon gets attached within ~2 s on a cold connect).
- [x] **P2: Surface `External` read-only rows from the daemon `status` response in the TUI list pane.** `status_icons` already defines the `⇪` glyph and `SurfaceState::External` variant, but `app::parse_status_row` only maps the five supervisor states (`launching`/`loading`/`ready`/`error`/`stopped`). The daemon's `status` method returns an `external` array (per plan: external `llama-server` processes detected via sysinfo with cmdline matching `*llama-server*`). Extend `ingest_status` to also parse the `external` array into `ManagedRow`s with `state: SurfaceState::External` so the list pane shows them read-only with the `⇪` glyph and only the `stop` action available.

---

- [x] **Unit 7: Right-pane tabs — Logs, Chat, Embed, Rerank**
...
**TODO follow-ups from Unit 7 review (2026-05-15 — commit `5529582` right-pane tabs):**

- [x] **P0: Wire text-input capture for right-pane tabs.** When `Focus::RightPane` is active, `handle_key` falls through to `action_for(Focus::RightPane, ...)` which silently drops unbound keystrokes. Chat/embed/rerank input fields can never be populated. Fix: add per-tab `Focus` variants (`Focus::ChatInput`, `Focus::EmbedInput`, `Focus::RerankInput`) or a tab-aware text-capture dispatch so typed characters reach `app.chat.prompt`, `app.embed.input`, and `app.rerank.query`. At minimum, forward alphanumeric + backspace + enter to the active tab's input buffer. Cover with a smoke test that types "hello" into the chat prompt and asserts it appears in the render.
- [x] **P0: Wire `Ctrl+Enter` to send the chat prompt via `spawn_chat_stream`.** The plan specifies `Ctrl+Enter` as the send hotkey but there is no `Action::SendChat` or `Ctrl+Enter` binding in `RIGHT_PANE_BINDINGS`. The `spawn_chat_stream` function exists and the integration test proves it works, but nothing in the TUI run loop calls it. Fix: add a `Ctrl+Enter` binding that reads `app.chat.prompt`, calls `spawn_chat_stream(app.focused_managed().port, focused_name(), prompt)`, drains the returned `mpsc::Receiver` into `app.chat` via a periodic `try_recv` in the render loop, and pushes final state on `Finished`/`Error`. Cover with a TestBackend smoke test that presses `Ctrl+Enter` and asserts the streaming state appears.
- [x] **P0: Wire `Enter` submit for embed and rerank tabs.** The embed tab status bar says "Enter to embed" and the rerank tab says "Enter to rank", but `Enter` in `Focus::RightPane` dispatches to `action_for` which has no `Submit` binding for the right pane. Fix: add `Enter` bindings for `ChatInput`, `EmbedInput`, `RerankInput` sub-focuses that call `oai_client::embed` / `oai_client::rerank` asynchronously and push results into `app.embed` / `app.rerank`. Cover with smoke tests that assert dim/preview for embed and ranked scores for rerank. (The OAI client primitives already pass integration tests; this is purely wiring them to keyboard input.)
- [x] **P1: Populate the Logs tab from the daemon's `logs_tail` IPC method.** The tab renders a wireframe with an empty-state hint (`src/tui/tabs/logs.rs:46-53`) and `build_logs_state` explicitly returns `LogsTabState::default()` with a comment deferring to Unit 8 (`src/tui/right_pane.rs:91-97`). The daemon's `logs_tail` handler already exists (`src/ipc/methods.rs:255`) and integration tests drive it. Fix: add a `LogsTick` variant to `RefreshTick`, poll `logs_tail` for the focused model (when one is selected and has an active launch), deliver lines into `app`-owned per-launch log buffers, and render them via the existing `LogsTabState` viewport with the `auto_scroll` flag.
- [x] **P1: Add the `r` hotkey for toggling think-block collapse in the Chat tab.** The plan calls for `r` to toggle per-message ` thinking... response` collapse (R32). `ChatTabState.collapse_thinks` exists and `ooi_client::collapse_think_blocks` works, but there is no `Action::ToggleThinkCollapse` or `r` binding. Add a binding in `RIGHT_PANE_BINDINGS` (scoped to `Focus::ChatInput` or a parallel `ChatView` focus) and wire it to flip `app.chat.collapse_thinks`.
- [x] **P1: Drain the streamed chat response into `app.chat` via a non-blocking channel poll.** `spawn_chat_stream` returns an `mpsc::Receiver<ChatStreamMsg>` but no task reads from it. The render loop needs a `try_recv` drain path that appends `Delta` to `app.chat.response`, calls `mark_finished` on `Finished`, and shows an error toast on `Error`. Store the receiver handle in `app.chat` (e.g. `stream_rx: Option<mpsc::Receiver<ChatStreamMsg>>`) so it outlives the spawn.
- [x] **P2: Add multi-field input support to the Rerank tab.** The tab UI shows a query field and candidate list, but `RerankTabState` only has `query: String` and `candidates: Vec<String>` with no per-field cursor state or `Tab`-cycling between them. The status bar says "type candidate text then Tab to stage" but no Tab handler exists. Add a `field` cursor (Query / CandidateInput) to `RerankTabState`, a `Tab` cycle action that stages the current candidate text and clears the input, and render both the query prompt and the in-progress candidate with the caret.
- [x] **P2: Add an auto-scroll toggle hotkey for the Logs tab.** `LogsTabState.auto_scroll` exists but there is no keybinding to toggle it. Add `Action::ToggleAutoScroll` bound to a key (e.g. `Space` or `s`) and wire it to flip `auto_scroll` when `Focus::RightPane` and `app.right_tab == RightTab::Logs`.
- [x] **P2: Add per-tab focus states for right-pane text input.** Currently the only right-pane focus is `Focus::RightPane`, but each tab has its own input surface (chat prompt, embed text, rerank query). Adding `ChatInput`, `EmbedInput`, `RerankInput` variants to `Focus` (with a fallback `RightPane` for non-input tab views like logs) lets `handle_key` direct alphanumerics to the right buffer. This is a prerequisite for the P0 text-input fix; file as P2 because the scoping is more complex and can be deferred until the P0 input path is designed.
- [x] **P2: Add `Ctrl+Enter` and `/` keybindings to the right-pane help bar hints.** The plan documents `Ctrl+Enter` for send and the chat tab status line already mentions it. Add explicit bindings to the `RIGHT_PANE_BINDINGS` table (or the new per-tab focus tables) so the help bar renders them. Also add a binding for `Esc` to clear embed state when the tab already shows results.

---

- [x] **Unit 8: CLI non-interactive subcommands + JSON outputs**
...
**TODO follow-ups from Unit 8 review (2026-05-15 — commit `6b7b460` CLI subcommands):**

- [x] **P1: Include daemon PID, uptime, and active connection count in `status --json`.** The plan's approach says `status` returns "PID + uptime + #connections + #managed models." The current `status_json` output only has `models`, `external`, `gpu` — missing daemon-level health metrics. Add a `daemon` key to the JSON output with `pid`, `uptime_seconds`, and `active_connections`, sourced from the daemon's existing `version` IPC method (which already reports started_at via `elapsed`). The human formatter should also print a daemon-health preamble line next to the GPU label.
- [x] **P1: Allow `stop` to target external (unmanaged) `llama-server` processes.** The plan says external processes accept `stop` only. The `stop` handler uses `resolve_running` which scans `snap.models`, not `snap.external`. External processes (with PIDs like `ext-<pid>`) can't be stopped through the CLI. Fix: extend `resolve_running` (or add `resolve_external`) to match by `ext-NNN` prefix or by PID, then send `stop_model` with the matched `launch_id`. Cover with an integration test that surfaces an external row in status and stops it.
- [x] **P2: Add `--json` flag to `presets list`.** The plan says "Every read command supports `--json`." `PresetsAction::List` in `cli_args.rs` has no `json` field and the handler always prints TSV. Add a `json: bool` field to the `List` variant, pass it to the handler, and emit `pretty_json(&preset_list_json(&arr))` when set. Cover in `tests/cli_integration_test.rs` with a `--json` assertion on the preset list after a save.
- [x] **P2: Add integration test for `logs --follow` with daemon shutdown.** The plan's edge case "logs --follow and daemon shuts down → CLI prints 'daemon disconnected' and exits 65" has no coverage. The handler code path exists but no test exercises it. Spawn a daemon, launch a model, start `logs --follow` in a detached task, shut down the daemon, and assert the follow task exits with code 65.
- [x] **P2: Add integration test for `start --preset <name>` chaining.** The preset CRUD is tested in isolation (`presets_save_list_delete_round_trip`) and `start` is tested with explicit flags, but there's no end-to-end test that chains `presets save` → `start --preset <name>` and asserts the preset's ctx/reasoning/advanced flags reach the supervisor (verifiable via the `status` response's model params or by checking that the fake server received the expected port/endpoint).
- [x] **P2: Add integration test for `start` ctx-exceeds-native warning and duplicate-instance launch.** The plan's edge cases "start with --ctx exceeding native_ctx → exits 0 with a warning" and "start for a model already running → second instance on a new port; both visible in status" have no coverage. Add a test fixture with `native_ctx: 4096` that passes `--ctx 32768` and asserts exit 0, then add a second start of the same model and assert `status` shows two models with distinct ports.

---

### Phase E — Release

- [x] **Unit 9: Distribution, packaging, docs**
