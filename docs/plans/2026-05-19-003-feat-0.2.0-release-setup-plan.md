...
Today the only path to a working llamadash binary is `git clone && cargo install --path .`. The `Cargo.toml`'s `repository`, `homepage`, and `documentation` URLs point at `github.com/llamadash/llamadash` — a non-existent repo under an aspirational org name; the local working tree has no `origin` remote at all. The README pre-announces three install channels (`cargo install`, Homebrew tap, prebuilt binaries) that don't yet exist. There is a `.github/workflows/release.yml` that builds tarballs on tag and uploads them to GH Releases, but nothing pushes to crates.io, no tap repo exists, no install script, no marketing site, no CNAME on `llamadash.cli.rs`.

After this plan lands, a user on a fresh macOS or Linux machine runs exactly one of three commands and has `llamadash` on `$PATH` in under thirty seconds; a maintainer running `git tag v0.2.0 && git push --tags` triggers a fully automated release chain with zero post-tag manual steps. (See origin: `docs/brainstorms/2026-05-19-release-setup-requirements.md` → Success Criteria.)
...
- **R81** First-push migration to `github.com/llamadash-rs/llamadash` with full URL cleanup. (Unit 1)
- **R82** Supporting repos: `homebrew-llamadash`, `llamadash-rs.github.io`, `.github` org profile. (Unit 1, 4, 5)
...
- **R87** Crate name `llamadash` confirmed available; `llamadash-cli` fallback retained as race-guard. (Unit 1)
...
- **R90** `brew install llamadash-rs/llamadash/llamadash` + `--HEAD` source-build branch. (Unit 4)
...
- crates.io publishing docs for token scopes — the `CRATES_IO_TOKEN` should be a per-crate token (`llamadash`) once the crate exists, not a global account token; first publish uses a scoped one-time token created during Unit 2 setup.
...
- **Install script's served copy is a content-verified mirror, not a redirect.** GitHub Pages can't serve HTTP 30x redirects that `curl` honors, so `llamadash.cli.rs/install.sh` is regenerated by the site repo's bump workflow by fetching the GH Release asset and SHA-256-verifying before commit. Trust boundary is the bump workflow's verification step; branch protection on `.github/workflows/bump.yml` plus required reviews on workflow changes is the operational mitigation. (See origin: R91, Key Decision #4.)
...
- **No PATH mutation in install.sh.** Print a hint if `$LLAMADASH_INSTALL_DIR` (default `~/.local/bin`) isn't on `$PATH`; never edit `~/.zshrc`, `~/.bashrc`, `~/.profile`. Matches rustup's recent posture; predictable.
...
- **Site structure is opencode.ai-shaped, but copy and visual identity are llamadash-original.** Hero + multi-tab install + asciinema + features grid + social proof + why-local + FAQ + footer is the *layout pattern*; the words, asciinema content, and Catppuccin palette are the *brand identity*. (See origin: R96, Key Decision #1.)
...
- **What is the pre-tag guard for crate-name availability?** `curl -fsSL https://crates.io/api/v1/crates/<name>` returns 404 (unclaimed) or owner-list (must intersect llamadash-rs org members). 200 with non-member owner triggers the `llamadash-cli` rename fallback. (Origin R86 amended.)
...
- **Brew formula DSL version.** `class Llamadash < Formula ... end` block form vs newer `formula!` macro. Pin against `brew style` lint output during Unit 4.
...
```
github.com/llamadash-rs/
├── llamadash                       ← main source (this repo, first-pushed in Unit 1)
...
│
├── homebrew-llamadash              ← brew tap (Unit 4)
│   ├── Formula/llamadash.rb        ← binary formula, --HEAD branch
│   └── .github/workflows/bump.yml  ← repository_dispatch handler → rewrites formula → commits
│
├── llamadash-rs.github.io          ← marketing site (Units 5 + 6)
│   ├── src/                        ← Astro project
│   ├── public/install.sh           ← SHA-256-verified copy of GH Release asset (R91)
│   ├── public/install.sh.sha256
│   ├── public/demo.cast            ← asciinema cast
│   ├── CNAME                       ← llamadash.cli.rs
│   └── .github/workflows/
│       ├── deploy.yml              ← astro build → actions/deploy-pages
│       └── bump.yml                ← dispatch handler → fetch+verify install.sh + update labels → commit
│
...
sequenceDiagram
    ...
    participant Main as llamadash/release.yml
    participant GHR as GitHub Releases
    participant Tap as homebrew-llamadash/bump.yml
    participant Site as llamadash-rs.github.io/bump.yml
    ...
    par Phase 1 (reversible, parallel)
        ...
        Tap->>Tap: rewrite Formula/llamadash.rb, commit
    and
...
| Grep guard for old `llamadash/llamadash` URLs | ci.yml (new, Unit 2) | Build red — URL drift | R86 |
...
- [ ] **Unit 1: Repo migration + Cargo.toml + URL cleanup** *(partial — URL/metadata cleanup landed in `019fa78`. Org-admin actions documented in `docs/runbooks/release-0.2.0-bootstrap.md`; pending: create the four `llamadash-rs/*` GitHub repos and configure `CRATES_IO_TOKEN` + `GH_BUMP_TOKEN` secrets per the runbook)*

**Goal:** First-push the local working tree to `github.com/llamadash-rs/llamadash`, fix every URL reference, add missing Cargo metadata, create the empty supporting repos, prepare CHANGELOG.
...
**Files:**
- Modify: `Cargo.toml` (`repository`, `homepage`, `documentation` → `https://github.com/llamadash-rs/llamadash`; add `categories = ["command-line-utilities"]`; confirm `keywords` ≤ 5; review `exclude` list)
- Modify: `README.md` (every `github.com/llamadash/llamadash` reference; pre-1.0 binaries-not-yet-published warning rewritten to link the new install methods or marked as accurate at tag time)
...
- Create: new GitHub repos (org-admin action, not source-controlled): `llamadash-rs/llamadash` (target of the push), `llamadash-rs/homebrew-llamadash` (empty, populated by Unit 4), `llamadash-rs/llamadash-rs.github.io` (empty, populated by Unit 5), `llamadash-rs/.github` (optional, with a one-paragraph `profile/README.md`)
- Test: none directly — verified by Unit 2's grep guard catching any remaining old-URL references.

**Approach:**
- The local tree currently has no `git remote`. Add `origin git@github.com:llamadash-rs/llamadash.git`, push `main`.
...
- Create `CRATES_IO_TOKEN` and `GH_BUMP_TOKEN` repo secrets before tagging — Unit 2's workflows are non-functional without them. `GH_BUMP_TOKEN` is a fine-grained PAT scoped to `contents:write` + `actions:write` on `homebrew-llamadash` and `llamadash-rs.github.io` only. (Upgrade to a scoped GitHub App is a 0.3.x consideration per origin §Outstanding Questions.)

**Patterns to follow:**
- `AGENTS.md` §"Docs stay in sync with code" — every file affected by the URL change is touched in the same commit.
- Conventional-commit prefix (per `AGENTS.md` §Conventions): commit subject `chore(release): migrate to llamadash-rs org` or similar; body lists the touched files.

**Test scenarios:**
- Happy path: after Unit 1 commit, `grep -r 'llamadash/llamadash' --include='*.md' --include='*.toml'` returns no matches except in `docs/brainstorms/` and `docs/plans/` (those preserve the historical record).
...
**Verification:**
- The new `llamadash-rs/llamadash` repo on GitHub shows the project README rendered with no broken `github.com/llamadash/llamadash` links.
...
- Pre-tag guards in `ci.yml`:
  ...
  - `crate-name-availability`: a small shell step that `curl -fsSL https://crates.io/api/v1/crates/llamadash` and branches on HTTP code: 404 = unclaimed (pass), 200 = inspect `owners` array and cross-reference against `gh api orgs/llamadash-rs/members --jq '.[].login'` — pass if intersection is non-empty, fail otherwise. The cross-reference uses `GH_BUMP_TOKEN` or the workflow's default token.
  - `url-grep-guard`: `! grep -rn 'llamadash/llamadash' Cargo.toml README.md AGENTS.md CONTRIBUTING.md` — refuse-on-match.
...
- Error path: `crate-name-availability` finds `llamadash` owned by a non-org user. The workflow fails with a clear message pointing to R87's `llamadash-cli` fallback path.
- Error path: `url-grep-guard` finds a residual `github.com/llamadash/llamadash` reference. The job lists the matching files and fails.
...
- Version resolution: if `LLAMADASH_VERSION` env var or `--version <tag>` flag is set, use it; else `curl -fsSL https://api.github.com/repos/llamadash-rs/llamadash/releases/latest | jq -r '.tag_name'` — but jq is not always installed, so fall back to grep+sed on the JSON. (Bats fixture provides both jq-present and jq-absent cases.)
...
- Extract `tar -xzf` to a temp dir, then `mv` the binary atomically to `$LLAMADASH_INSTALL_DIR/llamadash` (default `$HOME/.local/bin/llamadash`). `mkdir -p` the target if missing.
- Idempotence: before download, check if `$LLAMADASH_INSTALL_DIR/llamadash` exists, compute its SHA-256 against the registered target's expected, and short-circuit on match.
- PATH-on-path hint: `case ":$PATH:" in *":$install_dir:"*) ;; *) echo "Note: $install_dir is not on \$PATH..." ;; esac`. No file edits.
- Flags: `--version`, `--prefix`, `--quiet`, `-h | --help`. Env-var equivalents: `LLAMADASH_VERSION`, `LLAMADASH_INSTALL_DIR`, `LLAMADASH_QUIET`.
...
**Test scenarios:**
- Happy path: on Linux x86_64, install.sh detects target, fetches latest release tag, downloads + verifies + extracts to `~/.local/bin/llamadash`. Repeat for each of the four targets via Bats matrix.
- Happy path: `LLAMADASH_VERSION=v0.2.0 sh install.sh` pins to that tag; the resolution step skips the latest-release API call.
...
- Edge case: `$LLAMADASH_INSTALL_DIR` set to an unwriteable path; install.sh refuses with a clear error before downloading.
- Edge case: `$PATH` does not contain `$LLAMADASH_INSTALL_DIR`; install.sh prints the hint line.
...
- A manual smoke run: against the test tag from Unit 2 (`v0.0.0-rc1`), `curl -fsSL https://github.com/llamadash-rs/llamadash/releases/download/v0.0.0-rc1/install.sh | sh` installs cleanly on a fresh Ubuntu container.

---

- [ ] **Unit 4: Homebrew tap repo bootstrap + auto-bump** *(scaffolded locally at `../homebrew-llamadash/`; pending: org-admin creates the GitHub repo and first-pushes per `docs/runbooks/release-0.2.0-bootstrap.md`)*

**Goal:** Stand up `llamadash-rs/homebrew-llamadash` with an initial binary formula and a `bump.yml` workflow that listens for `repository_dispatch` events from the main repo's release workflow and rewrites the formula on each new tag.

**Requirements:** R82, R89, R90.

**Dependencies:** Unit 1 (the tap repo must exist on GitHub). Develop-time independent of Units 2, 3, 5.

**Files (in `llamadash-rs/homebrew-llamadash` repo — not the main repo):**
- Create: `Formula/llamadash.rb` — initial formula referencing v0.2.0 (placeholder hashes until the test-tag run produces real ones).
- Create: `.github/workflows/bump.yml` — `on: repository_dispatch: types: [bump]` workflow.
- Create: `README.md` — short installation instructions, link back to main repo.
- Create: `LICENSE` — match main repo (MIT).
- Test: no test files in the tap repo; verification is `brew install --force llamadash-rs/llamadash/llamadash` working against the test-tag.

**Approach:**
- Formula structure: `class Llamadash < Formula` with `desc`, `homepage`, `version`, `license`, `head "..." do ... end`, and four `if Hardware::CPU.arm? OS.mac?` (or `if OS.linux? && Hardware::CPU.intel?` etc.) branches setting `url` + `sha256` per target. The `head` branch fetches `github.com/llamadash-rs/llamadash.git` and builds from source with `system "cargo", "install", *std_cargo_args` so users opting into `--HEAD` get pre-release builds.
- `bump.yml` reads `client_payload.version` and `client_payload.sha256_<target>` × 4 from the dispatch, runs a small Ruby or sed script that rewrites the version + sha256 fields in `Formula/llamadash.rb`, commits with `Bot Llamadash Release <noreply@github.com>` (or the maintainer's identity if branch protection requires signed commits), and pushes.
...
**Test scenarios:**
- Happy path: pushing a `repository_dispatch` event manually (`gh api repos/llamadash-rs/homebrew-llamadash/dispatches -F event_type=bump -F 'client_payload[version]=0.2.0' -F 'client_payload[sha256_linux_x64]=<hash>' ...`) rewrites the formula and commits.
- Happy path: `brew install llamadash-rs/llamadash/llamadash` against the test-tag fetches the tarball, verifies the SHA-256 declared in the formula, installs the binary to `Cellar`, links to `bin/llamadash`.
- Happy path: `brew install --HEAD llamadash-rs/llamadash/llamadash` clones the main repo and builds from source via cargo.
- Edge case: a dispatch event with malformed `client_payload` (missing a SHA-256 for one target). The workflow refuses to commit and surfaces the error.
- Error path: a SHA-256 in the dispatch payload doesn't actually match the tarball at the URL. Brew install fails at fetch time with a checksum mismatch — desired behavior; the tap doesn't get to install a poisoned binary.
- Error path: the formula has a syntax error after rewrite. The commit still happens (the bump workflow doesn't lint), but installs fail with `brew style`-detectable errors. Mitigation: add a `brew style Formula/llamadash.rb` step to `bump.yml` that fails the workflow before commit. Adding this is recommended.

**Verification:**
- The tap repo exists, has the four files listed above, and `brew tap llamadash-rs/llamadash && brew search llamadash` returns a result.
- Against the test-tag from Unit 2: `brew install llamadash-rs/llamadash/llamadash` works on macOS and on Linux (with Linuxbrew).
- A manual `gh api ... dispatches` invocation produces a commit on the tap repo's `main` that updates the formula.

---

- [ ] **Unit 5: Website scaffolding + Catppuccin theme + dispatch handler** *(scaffolded locally at `../llamadash-rs.github.io/`; `npm run build` clean, 440 KB dist; pending: org-admin creates the GitHub repo and first-pushes per `docs/runbooks/release-0.2.0-bootstrap.md`)*

**Goal:** Stand up `llamadash-rs/llamadash-rs.github.io` as an Astro 4 project with Tailwind + asciinema-player wired, the full Catppuccin Macchiato palette tokenized, the deploy workflow publishing to GitHub Pages, and the `bump.yml` handler ready to verify + commit `install.sh` from each new GH Release. **Site content (the actual page) lands in Unit 6; this unit is the chassis.**
...
**Files (in `llamadash-rs/llamadash-rs.github.io` repo):**
- Create: `package.json` — Astro 4 latest stable + Tailwind 3 latest stable + `asciinema-player` + `@astrojs/tailwind`.
- Create: `astro.config.mjs` — Tailwind integration, `site: 'https://llamadash.cli.rs'`, no SSR (`output: 'static'`).
...
- Create: `public/CNAME` — exactly `llamadash.cli.rs`, one line, no trailing newline (GitHub Pages quirk).
...
- Create: `public/install.sh` — placeholder containing only `#!/bin/sh\necho "no release yet — see github.com/llamadash-rs/llamadash"` until the first dispatch from Unit 2 overwrites it.
...
- Create: `.github/workflows/bump.yml` — listens for `repository_dispatch: types: [bump]`. The `client_payload` carries `version` (e.g., `v0.2.0`) and the SHA-256 hashes; the workflow constructs the asset URL from the version: `https://github.com/llamadash-rs/llamadash/releases/download/${VERSION}/install.sh`. Fetches `install.sh` + `install.sh.sha256` from that URL, runs `shasum -a 256 -c install.sh.sha256` against the downloaded `install.sh`, aborts on mismatch, writes the file to `public/install.sh` (prepending a provenance comment header: `# Source: github.com/llamadash-rs/llamadash@<tag>/scripts/install.sh, SHA-256 verified at bump time`), commits both files plus a version-string update to `src/data/version.ts` (used by Unit 6's install-tab labels). Push triggers `deploy.yml`.
- Create: `README.md` — short, points at `llamadash.cli.rs` and the main repo.
...
- Happy path: pushing to `main` triggers `deploy.yml`; the deploy succeeds within the 30s target; the deployed site renders at `llamadash-rs.github.io` (and at `llamadash.cli.rs` after Unit 7 lands DNS).
- Happy path: a manually-fired `repository_dispatch` event with a valid `install.sh` + matching `install.sh.sha256` results in a commit that updates `public/install.sh` and `src/data/version.ts`.
...
- Integration: after Unit 5 + Unit 6, the site URL `llamadash-rs.github.io` returns 200 and the hero install-tab labels match the version in `src/data/version.ts`.
...
- A test `repository_dispatch` fired via `gh api` produces a commit and triggers a deploy.

---

- [ ] **Unit 6: Website content + asciinema cast + FAQ + accessibility** *(content authored in `../llamadash-rs.github.io/src/`; demo.cast + og-image + hero static are procedurally-generated placeholders pending real-recording exit criterion; Lighthouse pass deferred until live on Pages)*
...
**Test scenarios:**
- Happy path: visiting `llamadash-rs.github.io` (or `llamadash.cli.rs` after Unit 7) renders all eight sections in Macchiato palette.
...
- Pushing to `main` deploys; the live site at `llamadash-rs.github.io` shows all eight sections.
...
- [ ] **Unit 7: CNAME flip + first real tag + smoke verification** *(pre-tag file changes prepared: Cargo.toml at `0.2.0`, CHANGELOG `[Unreleased]` → `[0.2.0]` promotion, README install commands now reference live channels + llamadash.cli.rs. Tag-push, cli.rs DNS PR, and fresh-box smoke testing remain pending Unit 2 + org-admin bootstrap)*

**Goal:** Land `llamadash.cli.rs → llamadash-rs.github.io` DNS via PR to the community-managed `cli.rs` zone-config repo, flip the site to serve under the branded URL, cut the actual `v0.2.0` tag, and smoke-test every install path on a fresh box. This is the public-launch unit.
...
- Modify: `llamadash-rs/llamadash-rs.github.io` repo's `public/CNAME` — already committed in Unit 5; this unit's role is to confirm it's still `llamadash.cli.rs` and that the file is in `dist/` after build.
- Modify: `llamadash/README.md` — replace any "binaries not yet published" disclaimer with the three live install commands; mention `llamadash.cli.rs` as the marketing URL.
- Modify: `llamadash/Cargo.toml` — bump `version = "0.2.0-dev"` → `version = "0.2.0"`.
- Modify: `llamadash/CHANGELOG.md` — promote `[Unreleased]` content to `[0.2.0] — 2026-MM-DD`. (This is the canonical CHANGELOG promotion for the release; Unit 1 deliberately leaves it untouched so the final pre-tag commit owns it.)
- External: PR to the `cli.rs` zone-config repo adding a CNAME record `llamadash IN CNAME llamadash-rs.github.io.`.

**Approach:**
- Open the cli.rs PR first; while waiting for it to merge, the site is already live at `llamadash-rs.github.io` (Units 5+6). Marketing copy that mentions `llamadash.cli.rs` is held until DNS resolves.
- Once cli.rs DNS resolves: verify with `dig llamadash.cli.rs` returning a CNAME pointing at `llamadash-rs.github.io`. GitHub Pages auto-provisions an HTTPS cert via Let's Encrypt within a few minutes of CNAME working.
...
**Test scenarios:**
- Happy path: `dig llamadash.cli.rs` returns a CNAME to `llamadash-rs.github.io`; `curl https://llamadash.cli.rs` returns 200 with valid TLS.
...
- Happy path on fresh Ubuntu: `cargo install llamadash` succeeds; binary runs; `llamadash --version` shows `0.2.0`.
- Happy path on fresh Ubuntu: `curl -fsSL https://llamadash.cli.rs/install.sh | sh` succeeds; binary on `~/.local/bin/llamadash`.
- Happy path on fresh macOS: `brew install llamadash-rs/llamadash/llamadash` succeeds; binary runs.
- Happy path on fresh macOS: `curl -fsSL https://llamadash.cli.rs/install.sh | sh` succeeds; no Gatekeeper prompt (since curl path is quarantine-free per the origin's macOS analysis).
- Edge case: cli.rs DNS takes hours/days to propagate after PR merge. Marketing copy that mentions `llamadash.cli.rs` is held; release proceeds with `llamadash-rs.github.io` as the temporarily-canonical URL; copy is amended once DNS is live.
...
**Verification:**
- The three transcripts from fresh-box smoke testing each show a working `llamadash --version 0.2.0` line at the end.
- The GitHub Release notes link to the transcripts (or include them as collapsed `<details>` blocks).
- `https://llamadash.cli.rs` returns 200 with valid TLS and renders the Catppuccin Macchiato site.
...
- **Interaction graph:** Three cross-repo automation paths — main → tap (via `repository_dispatch`), main → site (via `repository_dispatch`), main → crates.io (via `cargo publish`). Each carries a token that is a leak risk; mitigations are documented in origin §Outstanding Questions (GH App migration deferred to 0.3.x). The release workflow's polling step (`gh run watch`) introduces a runtime dependency on the GitHub Actions API being responsive.
- **Error propagation:** Failures in the release pipeline are surfaced via GitHub Actions UI; phase 1 failure aborts phase 2; phase 2 failure leaves a recoverable partial state (tap + site committed, crates.io empty). Install.sh failures surface via distinct exit codes (1 generic, 2 checksum, 64 unsupported platform). Bump workflows abort on verification failure without committing — the site / tap stays on the previous version until the issue is fixed.
...
| CNAME for `cli.rs` takes longer than expected to provision; community-zone PR sits in review queue for days. | Medium | Low | Site is live at `llamadash-rs.github.io` from Unit 5 onward; marketing copy and crate description that mention `llamadash.cli.rs` are held until DNS resolves; release is not gated on cli.rs DNS. README install commands at tag time use whichever URL is live (either `llamadash.cli.rs` or fallback to the GH Releases URL directly — both work for install.sh). A late DNS flip is purely additive; nothing breaks. |
...
| `llamadash` crate name gets claimed by another publisher between brainstorm and tag. | Low | High (rename cascade) | Pre-tag CI guard (Unit 2's `crate-name-availability`) catches this; falls back to R87's `llamadash-cli` path; all dependent surfaces (README install commands, brew formula, install.sh hint text, site install tabs) auto-fall-back via the version-bump commit pattern. Mitigation cost: ~1 day of rename work, all in YAML/markdown — no Rust code changes since the binary keeps its short name. |
