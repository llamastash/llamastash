...
| Non-replayable knobs | `volatile_native_knobs` names the knobs a backend treats as a here-and-now judgement rather than a preference; `backend_knobs_for_persist` drops them (alongside the daemon's own `auto_set`) so a one-off `--preset` cannot silently become permanent |
